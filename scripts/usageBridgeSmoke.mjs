const baseUrl = (process.env.BRIDGE_BASE_URL ?? 'http://127.0.0.1:8787').replace(/\/$/, '');
...
function makePayload(payloadDate, totalScreenMs = 3600000) {
  return {
    schemaVersion: 1,
    owner,
    device: {
      name: 'Redmi smoke test',
      platform: 'android',
      osSkin: 'MIUI/HyperOS',
    },
    date: payloadDate,
    ...
    summary: {
      totalScreenMs,
      unlockCount: 12,
      firstUsedAt: `${payloadDate}T08:12:00+08:00`,
      lastUsedAt: `${payloadDate}T20:30:00+08:00`,
    },
    apps: [
      {
        packageName: 'com.codeandpurrs.smoke',
        appName: 'CodeAndPurrs Smoke',
...
async function main() {
  ...
  await requestJson('ping', buildUrl('/api/usage/ping'), {
    method: 'POST',
  ...
  if (latestBody.payload?.date !== date) {
  ...
  if (dayBody.payload?.summary?.totalScreenMs !== 3600000) {
    throw new Error('day returned unexpected totalScreenMs');
  }

  const trendBody = await requestJson('trend', buildUrl('/api/usage/trend', { owner, days: '7' }));
  if (!Array.isArray(trendBody.trend) || !trendBody.trend.some((item) => item.date === date)) {
