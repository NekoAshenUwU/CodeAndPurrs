...
- [ ] 未授权时显示"去开启"，不崩、不传空。
- [ ] 授权后，手动"立即上传"能让 VPS 收到一条符合 §4 契约的 JSON（`schemaVersion=1`；红米现役 app 发 `2`）。
...
## 4. 数据契约（两边唯一真相）｜`schemaVersion: 1 | 2 | 3`

> **v2（2026-08-23）**：红米现役 app 发 `2`。服务端两个都收，`3` 拒。
> v2 只加东西、不改旧字段语义，但 **`sessions[]` / `screenEvents[]` 的
> 时间口径变了**——见 §4.1b。旧 app 继续发 `1` 照常工作。
...
- **机主时区固定 `Asia/Kuching`（UTC+8，亚庇 Kota Kinabalu / 马来西亚沙巴）**，这是切"一天"的唯一基准。VPS 虽然在新加坡（也是 +8），但**不准用服务器本地时间**判断日期——一切以 payload 里带的时区为准。
...
```jsonc
{
  ...
  // 可选但强烈建议：每段前台会话（由 ACTIVITY_RESUMED/PAUSED 配对得来）。
  // 时间线"星河沙滩"的按 App 类别染色爪印、点击弹卡、深夜月亮、吐槽气泡都靠它。
  // 给不了就省略 → 网页退化成只用 hourly[24] 画朴素小路（无单 App 颜色/气泡）。
  // v2 起这是【增量窗口】而不是当天全量，且下限降到 1 秒——见 §4.1b。
  "sessions": [
    {
      "package": "com.xingin.xhs",
      "label": "小红书",
      "category": "social",         // social|work|entertainment|reading|tool|other
      "startAt": "2026-06-12T00:18:00+08:00",
      "endAt":   "2026-06-12T02:10:00+08:00",
      "open": false                 // v2 新增；true = 采集那刻还在前台
    }
  ],

  // v2 新增。屏幕开关 / 锁屏事件。判断睡眠全靠它：
  // 两次 interactive 之间的长空白 = 人没碰手机。
  // 同样是【增量窗口】。
  "screenEvents": [
    { "eventType": "keyguard_hidden", "at": "2026-06-12T07:14:00+08:00" }
  ],
...
### 4.1a v3 通知（`notifications[]`）

`schemaVersion: 3` 只加一样东西：

```json
"notifications": [
  { "package": "com.tencent.mm", "label": "微信", "at": "2026-08-30T14:23:07+08:00" }
]
```

**只有这三个字段，永远不会有标题和正文。** App 那边
（`NotificationLogger.kt`）压根不读 `sbn.notification.extras`——银行验证码和
别人发来的私信都会经过那个回调，不碰就不会存。予予要回答的是「她是不是在
被打扰」，不是「谁跟她说了什么」；想让他看内容，棠棠自己截图发过去。

**改这条要极其小心：往这里加字段等于扩大留在服务器上的隐私面。**
加之前先问她。

三类不上报（App 侧就滤掉）：App 自己的通知；常驻的（音乐/导航/下载——
它是「一直在那儿」不是「刚来的」，还会反复重发把真通知淹掉）；分组汇总
那条（会跟每条各发一次）。

服务端落 `notifications(package, label, ts, created_at)`，
`UNIQUE(package, ts)` 保证重复上报幂等。授权是 Android 的
「通知使用权」，跟「使用情况访问」是**两个不同的开关**，要分别给。

### 4.1b v2 增量语义（改这块必须同步改 `schemaVersion`）

v1 每次上报都是**当天全量快照**，服务端整份覆盖 `{date}.json` 就行。
v2 的 `sessions[]` / `screenEvents[]` 改成**增量窗口**：只包含「上次成功
上报之后 → `generatedAt`」这段时间里的事件。

这么改是因为系统事件有保留期，全量重扫既慢又会在两次同步之间留下缝。
代价是**服务端不能再整份覆盖**——覆盖一次，星河沙滩当天就只剩最后一段。

| | v1 | v2 |
|---|---|---|
| `summary` / `hourly` / `apps` | 当天全量 | **不变**，仍是当天全量 |
| `sessions` / `screenEvents` | 当天全量 | 增量窗口 |
| 会话下限 | ≥30s | **≥1s** |
| 服务端写日文件 | 整份覆盖 | 按 key 合并 |

几条硬规矩：

- **合并 key**：会话 `package|startAt`，屏幕事件 `eventType|at`。
  比较之前把时间戳**砍到整秒**（`2026-08-22T21:03:11.482+08:00` →
  `2026-08-22T21:03:11+08:00`）——同一条会话两次上报的小数位可能不一样，
  不归一化就会存成两条。归一化要用字符串手术，`new Date()` 会把 `+08:00` 弄丢。
- **`open` 的会话可以被覆盖，封了口的不行。** `open: true` 表示采集那刻
  app 还在前台、`endAt` 是拿同步时刻凑的；下次带真实 `endAt` 回来覆盖同一条。
  反过来不行——已经封口的记录不该被迟到的重复上报改回 open。
- **会话下限从 30s 降到 1s。** 夜里醒来摸一下手机可能只有十几秒，
  30s 门槛会把 `get_sleep_gap` 要找的夜醒整段砍掉。
  （`apps`/`hourly` 的当天聚合仍沿用 30s，不改既有统计口径。）
- **跨天的会话不切分**，保留原始时间戳，按 `startAt` 归属当天。
  首次运行会回溯 3 天，所以日文件只收 `startAt` 落在本日的；库里那份是全的。

### 4.1c 服务端落库（v2）

`server/usageBridgeServer.mjs` 除了写 `{date}.json`，还会把会话和屏幕事件
写进 `/root/data/dream_events.db` 的 `usage_sessions` / `screen_events`
两张表（`spawnSync('sqlite3')`，不加 npm 依赖），供棠予酿 MCP 的
`get_phone_sessions` / `get_sleep_gap` 查询。

这条链路**刻意独立于** `app_usage_snapshot` 那条同步——那边有一句
`apps is empty, skip`，而夜里没开 app 是常态，睡眠数据全在那时候。
...
**风格**：沿用首页 v2「仙气浪漫风」token —— 毛玻璃卡片、紫粉雾感渐变、发光、草棵可爱体；移动优先、**单列竖滑**、卡片堆叠，中心列宽 ~ 480–560px（像一台手机）。
**动效**：背景渐变呼吸、环填充、星屑飘散、肉垫浮动、爪印错峰淡入；全部走现有 `usePrefersReducedMotion` 兜底（关闭时一律静止到位、不撒粒子）。
**配色 token**：`--iris #9b7bc8`、`--iris-deep #5d4a7e`、`--sakura #f2a9c4`、`--blueviolet #7b8fe8`、`--glow`、`--cream`、`--card-strong`。
**类别马卡龙色**（爪印/肉垫/时长条共用）：社交=粉 `social`、工作=淡紫 `work`、娱乐=蜜橙 `entertainment`、阅读=薄荷、工具/其他=雾灰。
**字体**：标签用 `--cute`(草棵)、英文用 `--serif-en`(Crimson)、大数字用 `--round`(Fredoka)、猫咪点评用 `--chat`(龙珠)。

### ⓪ 会呼吸的时间背景（全局基底）
- 整页背景跟**真实当前时间**走，缓慢渐变（"足迹"本就是时间的故事，让页面也活在时间里）：
  - 清晨 5–9：奶橘 → 浅粉柔雾
  - 白天 9–17：天蓝 → 淡紫
  - 傍晚 17–20：粉紫烧霞晚霞
  - 深夜 20–5：深空蓝黑 + 细碎星点轻微闪烁
- 与姐妹站 **棠予酿**的深空风一脉相承（两站像姐妹）；深夜档基本就是深空皮肤。
- 实现：按本地小时取一组渐变停靠色，CSS 变量驱动、`transition` 慢过渡；星点用极轻的 canvas/CSS 粒子（reduced-motion 时星点静止、不闪）。
- 注意：背景在最底层，毛玻璃卡片浮其上；深夜档要调高卡片对比度，保证可读。

从上到下模块：

### ① 今日总时长 · 发光爪印活动环（主视觉 · 有小灵魂）
- SVG 径向环：轨道半透明，进度描边走 `iris → sakura` 渐变 + 外发光（drop-shadow `--glow`）。进度 = `totalForegroundMs` 相对柔性参考（默认 6h，或近 7 天个人峰值）。
- **环每闭合一整圈 → 撒一小圈星屑粒子飘散**（满 1 圈/2 圈… 各撒一次，入场动画跑到对应圈数时触发；reduced-motion 不撒）。
- **环心猫 mascot 按时长换状态**（数据一眼有表情）：
  - 轻度（<参考 60%）→ 伸懒腰晒太阳 😺
  - 偏多（60–100%）→ 捂着眼睛装看不见 🙈
  - 超标（>100%）→ 直接趴下装死 🫠
  - （素材：preferably 3 张 mascot 状态图；缺图先用同一张 + 表情角标降级。）
- 环心：状态 mascot + 大数字 `3ʰ12ᵐ`（Fredoka）+「今日」(草棵)；环起点缀一枚小爪印 🐾。
- 环下方对比药丸：`比昨天少 23m`（少=薄荷绿，多=暖琥珀）。
- 入场：环从 0 转到目标值（reduced-motion 直接到位）。

### ② 猫咪点评（v1 接 AI）
- 毛玻璃卡：左猫咪小头像，右语音气泡（龙珠体）。
- 文案来自 AI：网页把当天 `summary`+top apps 拼成上下文，POST 到现有聊天后端（`/api/chat`，"足迹点评猫"人设：高用量→心疼/护眼，深夜→催睡，轻量→夸夸）。**单条短回复，按 `date` 缓存当天**。
- 加载态：「猫咪正在看你今天的足迹…」配爪印 loader。

### ③ 爪印榜 · 坐肉垫的 Top Apps
- 别用表格。每行：**App 图标坐在一只圆滚滚的猫肉垫上**（肉垫染该 App 类别马卡龙色）+ 名称 + 右侧时长 + 身后拖一条**马卡龙渐变时长条**（宽度=占榜首百分比，条尾缀爪印）。
- **第一名肉垫最大、还微微上下浮动**（像在得意），更强毛玻璃 + 发光。
- 默认 Top 5，「展开看全部」展开剩余；无图标用爪印占位。

### ④ 爪印时间线 · 星河沙滩（第二主视觉）
- 24h 横带**别画成干巴巴的轴**：做成一条横向**渐变星河**，爪印就是踩在上面的脚印。
- **数据**：优先用 `sessions[]` —— 每段会话落一串爪印，按 `category` 染马卡龙色（社交粉/工作淡紫/娱乐蜜橙…）；没有 `sessions[]` 时退化用 `hourly[24]` 画单色密度小路（无分类色/无气泡）。
- **点一枚爪印 → 弹毛玻璃小卡**：显示 App 名 + 该段时长（+ 起止时刻）。
- **深夜特殊待遇**：本地 0:00–5:00 的爪印自带**一弯小月亮**、泛冷蓝微光 🌙 —— 熬夜证据可视化（"以后骂人有图有真相"）。
- 轻标注时段：夜 / 晨 / 午 / 晚高峰。

### ④-b 呼噜的吐槽气泡（缝进数据里的灵魂，拉开差距的一刀）
- 时间线上**某段用得太凶**（单 App 连续会话 ≥ 阈值，默认 90–120min）→ 该位置自动冒一个小猫咪气泡，软软吐槽：
  - 例：小红书连刷两小时 → 「在甜甜口袋待了好久呀~」
- 文案策略：先做**本地映射表**（按 `category`/常见包名给几句俏皮话，深夜段换"该睡啦"语气），稳定零成本；后续可升级成调 `/api/chat` 现生成。
- 一屏最多冒 1–2 个气泡（挑最凶的段），别糊屏；可点掉。
- 触发依赖 `sessions[]`；没有就不冒（朴素降级）。
...
4. Claude：猫爪足迹网页 ⓪ 时间背景 + ①–⑥ 模块（含星屑/肉垫/星河沙滩/吐槽气泡）+ 状态态。
5. 联调：真机红米推一天 → 网页出爪印。
