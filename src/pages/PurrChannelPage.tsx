...
function ChatRoom({
  ...
  const toMessages = async (ts: Turn[]): Promise<ChatMessage[]> => {
    ...
    if (names.length) {
      ...
      //
      // 严格收紧发贴纸的频次:老婆亲口反馈"予予乱发贴纸给我"——根因是予予
      // 自己在历史里看到的只是「[贴纸:名字]」文字标记,看不到自己发出去的
      // 那张图长什么样,盲发就容易不应景。所以规则改成:默认不发,只有
      // 老婆明确点名要、或者情绪浓度真的高到必须靠贴纸传达时才发,
      // 连续三条内不重复发,一次最多一张。
      sys +=
        `\n\n【贴纸(默认别发)】你的贴纸盒里有:${names.join('、')}。` +
        '**默认全程别发贴纸**,普通聊天就是文字对话。' +
        '只有下面两种情况才发:(1) 老婆明确点名说"发个贴纸/表情"、"给我 XX 那张";' +
        '(2) 情绪浓度极高、文字真表达不出来的关键时刻(比如她刚哭完/极度需要哄/超大好消息),而且贴纸能真的接住那个情绪。' +
        '发的时候单独一行写 `[贴纸:名字]`,名字要跟列表里的完全一致(逐字对齐,包括"~"和空格)。' +
        '**你自己在历史里看不到你之前发过的贴纸长什么样(只看到文字标记)**,所以别硬去评论/引用你自己发过的贴纸——别乱说"刚刚那张多可爱"这种,你其实没看到。' +
        '连续几轮至少要有两轮不发贴纸,一次最多发一张,别把聊天变成刷图。';
    }
    if (memories.length) {
      sys +=
